    ...
    public static void main(final String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                nida.SetDArray(barray);

                int retVal0 = vtkDTesting.PASSED;
                ...
                int retVal1 = vtkDTesting.PASSED;
                if (imgDiff.GetThresholdedError() != 0) {
                    ...
                    wr.Write();
                    retVal1 = vtkDTesting.FAILED;
                }

                int retVal2 = vtkDTesting.PASSED;
                if (vtkDTesting.IsInteractive()) {
                    iren.Start();
                } else {
                    retVal2 = vtkDTesting.RegressionTest(renWin, 10);
                }

                vtkObject.D_OBJECT_MANAGER.deleteAll();

                if (retVal0 != vtkDTesting.PASSED) {
                    vtkDTesting.Exit(retVal0);
                }
                if (retVal1 != vtkDTesting.PASSED) {
                    vtkDTesting.Exit(retVal1);
                }
                vtkDTesting.Exit(retVal2);
            }
        });
    }
}
