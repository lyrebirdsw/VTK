...
implements MouseListener, MouseMoveListener, MouseTrackListener, MouseWheelListener, KeyListener {
  ...
  public static int convertModifiers(int mods) {
    int modifiers = 0;
    if ((mods & SWT.SHIFT) != 0) modifiers |= d.awt.Event.SHIFT_MASK;
    if ((mods & SWT.CTRL) != 0) modifiers |= d.awt.Event.CTRL_MASK;
    if ((mods & SWT.ALT) != 0) modifiers |= d.awt.Event.ALT_MASK;
    return modifiers;
  }

  public d.awt.event.KeyEvent convertKeyEvent(org.eclipse.swt.events.KeyEvent e) {
    return new d.awt.event.KeyEvent(dummyComponent, 0, (long)e.time, convertModifiers(e.stateMask), e.keyCode, e.character);
  }

  public d.awt.event.MouseEvent convertMouseEvent(org.eclipse.swt.events.MouseEvent e) {
    int button = 0;
    if ((e.button == 1) || (e.stateMask & SWT.BUTTON1) != 0) button = d.awt.event.MouseEvent.BUTTON1;
    else if ((e.button == 2) || (e.stateMask & SWT.BUTTON2) != 0) button = d.awt.event.MouseEvent.BUTTON2;
    else if ((e.button == 3) || (e.stateMask & SWT.BUTTON3) != 0) button = d.awt.event.MouseEvent.BUTTON3;
    return new d.awt.event.MouseEvent(dummyComponent, 0, (long)e.time, convertModifiers(e.stateMask), e.x, e.y, e.count, false, button);
  }
